...
uint32_t temperatureTarget = DEFAULT_TEMP;
...
//! \return None.
//*****************************************************************************
void setTemperatureTarget(uint32_t newTemperatureTarget)
{
...
float getPComponent()
{
	uint32_t latestError = CircularArray_Peek(&circle, 0);
...
float getDComponent()
{
	int32_t latestError = CircularArray_Peek(&circle, 0);
	int32_t nextError = CircularArray_Peek(&circle, 1);
	int32_t deltaError = nextError - latestError;
