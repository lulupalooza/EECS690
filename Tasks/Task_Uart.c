...
float temperatureTarget;//room temperature @ 21.0C or 70.0F
...
extern void Task_UART_0( void *pvParameters ) {
	...
	while ( 1 ) {
		...
		//PSEUDO CODE: if temperatureTarget is extern
		/*if(temp_change == '+')
			temperatureTarget += 1;
		else if(temp_change == '-')
			temperatureTarget -= 1;*/
		//PSEUDO CODE: if temperatureTarget is NOT extern
		//if(temp_change == '+')
		//	setTemperatureTarget(getTemperatureTarget() + 1);
		//else if(temp_change == '-')
		//	setTemperatureTarget(getTemperatureTarget() - 1);
